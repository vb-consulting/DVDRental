...
public static class PgRoutineFilmNotInStock
{
    public const string Name = "public.film_not_in_stock";
    public const string Query = $"select p_film_count from {Name}($1, $2)";

    /// <summary>
    /// Executes sql function public.film_not_in_stock(integer, integer)
    /// </summary>
    ...
    public static IEnumerable<int?> FilmNotInStock(this NpgsqlConnection connection, int? pFilmId, int? pStoreId)
    {
    ...
    /// <summary>
    /// Asynchronously executes sql function public.film_not_in_stock(integer, integer)
    /// </summary>
    /// <param name="pFilmId">p_film_id integer</param>
    /// <param name="pStoreId">p_store_id integer</param>
    /// <returns>IAsyncEnumerable of int? instances</returns>
    public static async IAsyncEnumerable<int?> FilmNotInStockAsync(this NpgsqlConnection connection, int? pFilmId, int? pStoreId)
    {
        using var command = new NpgsqlCommand(Query, connection)
        {
            CommandType = System.Data.CommandType.Text,
